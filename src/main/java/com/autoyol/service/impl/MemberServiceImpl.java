        ...
        if (value == null && value == "") {
            ...
            if (list == null || list.size() == 0) {
                ...
                while (iter.hasNext()) {
                    ...
                    String res = "\r" + "redis用户信息如下：\n" + jedis.get("user:center:token:" + token);    // raids 通过key 查 value
                    str += res;
                }
                str = formatJson(str);
                result.setStatus(0);
            ...
            if (list == null || list.size() == 0) {
                String str = "";
                if (null != jedis.get("user:center:token:" + value)) {
                    str = "redis用户信息如下：\r" + jedis.get("user:center:token:" + value);
                    str = formatJson(str);
                    result.setStatus(0);
                    result.setMsg("success");
                    result.setData(str);
                    ...
                    result.setStatus(0);
                    result.setMsg("success");
                    result.setData("无此用户，请检查输入是否正确：" + value);
                }
                ...
                while (iter.hasNext()) {
                    ...
                    String res = "\r" + "redis用户信息如下：\n" + jedis.get("user:center:token:" + tokens);    // raids 通过key 查 value
                    str += res;
                }
                str = formatJson(str);
                result.setStatus(0);
